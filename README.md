...
In these usage examples, we use `Selenium` mode to extract SVG from
...
you away :rocket: Check also another
[showcase](https://www.kaggle.com/code/jacekpardyak/d3r-lib) on
`Kaggle`.
...
make(script = script, 
     ...
     style = style, 
     mode = "selenium", 
     dir = "man/logos/") |> knitr::include_graphics()
```

![](man/logos/file5a94360d1a47.svg)<!-- -->

## Available `make()` parameters

- script - JavaScript string

- style - (optional) CSS string

- html - (optional) HTML template string

- data - (optional) JSON string. This adds line
  `const data = <your_data>;` to the JavaScript.

- title - (optional) Title

- dir - (optional) Directory to store HTML/SVG file

- mode - (optional) Available modes: NULL, Jupyter, RStudio, Quarto,
  Shiny, Selenium

- width - (optional) iframe width for Shiny and Quarto

- height - (optional) iframe height for Shiny and Quarto

Function returns HTML/SVG document presentation appropriate to the
`mode`
...
- If mode = `quarto`, it will render in Quarto with the specified HTML
  iframe width/height.
...
## Minimal Working non-HTML Quarto Docs
...
For DOCX use:
    ...
    make(script = script, mode = 'selenium') |> knitr::include_graphics()
